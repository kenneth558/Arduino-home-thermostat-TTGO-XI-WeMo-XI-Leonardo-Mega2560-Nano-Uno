...
int idDHTLib::acquire() {
	if (state == STOPPED || state == ACQUIRED) {
		...
		// Check for pin to be high or abort
//        if (digitalRead(pin) == LOW) return IDDHTLIB_ERROR_ACQUIRING;
		delayMicroseconds(25);
		// Check for pin to be low or abort
//		if (digitalRead(pin) == HIGH) return IDDHTLIB_ERROR_ACQUIRING;
